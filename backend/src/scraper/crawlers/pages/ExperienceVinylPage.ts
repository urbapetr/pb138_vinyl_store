...
const getProductsFromPage = async (pageUrl: string): Promise<Array<Vinyl>> => {
  ...
  $('#product-grid .card').each( (i, data) => {
    ...
    const price = parseFloat($(data).find('.price-item--sale').text().trim().replace(/[^0-9.]/g, ''));
    const availableText = $(data).find('.quick-add__submit span:eq(0)').text().trim(); // === "Add to Cart";
    ...
    const available = availableText ? true : false;
...
const getProducts = async (pageStart?: number, pageLimit?: number): Promise<Array<Vinyl>> => {
  ...
  for (let i = (pageStart ?? 1); i < (pageLimit ?? pageCount); i++) {
    const data = await getProductsFromPage(`https://experiencevinyl.com/collections/vinyl-record-lps?page=${i}`)
    console.log("PARSED: ");
    console.log(data);
    vinyls = [... vinyls, ...data];
    await delay(defaultDelay);
